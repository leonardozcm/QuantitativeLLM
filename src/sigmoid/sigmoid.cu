...
__global__ void verify_gpu(const T *c, const T *a, int M, int N, int *ret)
{
    ...
    if ((*ret) && c[x_id * N + y_id] != (1/(1+expf(-a[x_id * N + y_id]))))
    {
        (*ret) = 0;
    }
}
...
// Kernels
template<typename T>
__global__ void sigmoid_native(const T* a, T* b, int N){
    int col_idx=blockIdx.x*blockDim.x+threadIdx.x;
    int row_idx=blockIdx.y*blockDim.y+threadIdx.y;

    int idx = row_idx * N + col_idx;
    b[idx] = 1/(expf(-a[idx])+1);
}
...
void test_with_dtype(qttbench::State& state){
    ...
    state.run(
        "sigmoid_native_native blocksize 32*8",
        [&](cudaStream_t s)
        {
            ...
            sigmoid_native<T><<<grid_size, block_size>>>(
                d_a, d_b, N
            );
